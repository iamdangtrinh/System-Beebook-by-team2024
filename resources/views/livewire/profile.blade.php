                        ...
                        <form wire:submit='handleEditProfile' class="d-flex flex-column gap-2 ">
                            <div class="w-100 d-flex justify-content-center flex-col">
                                <div class="image-upload">
                                    <label for="file-upload" style="cursor: pointer;">
                                        <img style="width: 100px; height: 100px; border-radius: 50%; border: 1px solid black"
                                            src={{ asset('storage/uploads/' . ($Avt === '' ? 'no_image.jpg' : $Avt)) }}
                                            alt="User Avatar">
                                    </label>
                                    <input type="file" wire:model.change="avatar" id="file-upload"
                                        wire:model.live="avatar" style="display: none;" accept="image/*">
                                </div>
                                @error('avatar')
                            ...
                            <div class="col-12 position-relative">
                                <div class="form-group">
                                    ...
                                    <input wire:model.live="name" class="rounded-1" id="CustomerEmail" autocorrect="off"
                                        autocapitalize="off">
                                </div>
                                @error('name')
                                    <span class="error text-danger">{{ $message }}</span>
                            ...
                            <div class="col-12 position-relative">
                                <div class="form-group">
                                    ...
                                    <input wire:model.live="phone" class="rounded-1" id="CustomerEmail"
                            ...
                            <div class="col-12 position-relative">
                                <div class="form-group">
                                    ...
                                    <input wire:model.live="email" class="rounded-1" id="CustomerEmail"
                                        autocorrect="off" autocapitalize="off">
                                </div>
                                @error('email')
                                    <span class="error text-danger">{{ $message }}</span>
                                    ...
                                    <ul class="list-group position-absolute w-100">
                                        @if ($chooseAddress)
                                            @foreach ($chooseAddress as $value)
                                                <li wire:click="addAddress('{{ $value['description'] }}')"
                                ...
                                <button @if (
                                    $errors->any() ||
                                        ($name === $result->name &&
                                            $phone === $result->phone &&
                                            $email === $result->email &&
                                            $address === $result->address)) disabled @endif type="submit"
                                    class="btn">Cập nhật</button>
